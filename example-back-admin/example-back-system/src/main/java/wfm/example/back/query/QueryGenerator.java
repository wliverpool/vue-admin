package wfm.example.back.query;
...
import wfm.example.common.dto.SysPermissionDataRuleDTO;
import wfm.example.common.enums.MatchTypeEnum;
import wfm.example.common.enums.QueryRuleEnum;
import wfm.example.common.util.DateUtils;
import wfm.example.common.util.ObjectConvertUtils;
import wfm.example.common.util.SqlInjectionUtil;
...
public class QueryGenerator {
    ...
    public static void installMplus(QueryWrapper<?> queryWrapper,Object searchObj,Map<String, String[]> parameterMap) {
		...
		*/
        ...
        Map<String, SysPermissionDataRuleDTO> ruleMap = getRuleMap();
            ...
            try {
                ...
                queryWrapper.and(andWrapper -> {
                    for (int i = 0; i < conditions.size(); i++) {
                        ...
                        if (ObjectConvertUtils.isNotEmpty(rule.getField())
                                && ObjectConvertUtils.isNotEmpty(rule.getRule())
                                && ObjectConvertUtils.isNotEmpty(rule.getVal())) {
     ...
     * @return
     */
    public static Map<String, SysPermissionDataRuleDTO> getRuleMap() {
        Map<String, SysPermissionDataRuleDTO> ruleMap = new HashMap<String, SysPermissionDataRuleDTO>();
        List<SysPermissionDataRuleDTO> list = DataAuthorUtils.loadDataSearchConditon();
        if(list != null&&list.size()>0){
            if(list.get(0)==null){
                return ruleMap;
            }
            for (SysPermissionDataRuleDTO rule : list) {
    ...
    private static void addRuleToQueryWrapper(SysPermissionDataRuleDTO dataRule, String name, Class propertyType, QueryWrapper<?> queryWrapper) {
    ...
    public static String convertRuleValue(String ruleValue) {
        String value = JwtUtils.getUserSystemData(ruleValue, null);
    ...
    public static String trimSingleQuote(String ruleValue) {
        if (ObjectConvertUtils.isEmpty(ruleValue)) {
            return "";
        }
    ...
    public static String installAuthJdbc(Class<?> clazz) {
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            if(ruleMap.containsKey(name)) {
                SysPermissionDataRuleDTO dataRule = ruleMap.get(name);
                QueryRuleEnum rule = QueryRuleEnum.getByValue(dataRule.getRuleConditions());
    ...
    public static String getAllConfigAuth() {
        ...
        String sql_and = " and ";
        for (String c : ruleMap.keySet()) {
            SysPermissionDataRuleDTO dataRule = ruleMap.get(c);
    ...
    private static String getDbType(){
        ...
        try {
            ISysBaseAPI sysBaseAPI = SystemConfig.getBean(ISysBaseAPI.class);
