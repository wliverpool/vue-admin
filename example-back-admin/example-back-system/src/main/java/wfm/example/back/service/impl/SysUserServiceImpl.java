...
import wfm.example.common.util.UUIDGenerator;
import wfm.example.common.vo.Result;
import wfm.example.common.vo.SysUserCacheVo;
import wfm.example.common.vo.SysUserDepVo;
...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements ISysUserService, UserDetailsService {
    ...
    public Result<?> resetPassword(String username, String oldpassword, String newpassword, String confirmpassword) {
        ...
        // PasswordUtils.encrypt(username, newpassword, user.getSalt());
...
//			// TODO URL规则有问题？
//			if (ObjectConvertUtils.isNotEmpty(po.getUrl())) {
...
//			}
    ...
    @Override
    public SysUserCacheVo getCacheUser(String username) {
        SysUserCacheVo info = new SysUserCacheVo();
...
//		this.save(user);  //保存角色的时候已经添加过一次了
        if(ObjectConvertUtils.isNotEmpty(selectedParts)) {
            ...
            for (String deaprtId : arr) {
                SysUserDepart userDepart = new SysUserDepart(user.getId(), deaprtId);
    ...
    public Result<?> checkUserIsEffective(SysUser sysUser) {
        ...
        if (CommonConstant.DEL_FLAG_1 == sysUser.getDelFlag()) {
            log.info("用户登录失败，用户名:" + sysUser.getUsername() + "已注销！");
            result.error500("该用户已注销");
            return result;
        }
        //情况3：根据用户信息查询，该用户已冻结
        if (CommonConstant.USER_FREEZE.equals(sysUser.getStatus())) {
            log.info("用户登录失败，用户名:" + sysUser.getUsername() + "已冻结！");
