package wfm.example.back.rule.impl;
...
import wfm.example.common.util.CodeGenerateUtils;
...
public class OrgCodeRule implements IFillRuleHandler {
    ...
    public Object execute(JSONObject params, JSONObject formData) {
        ISysDepartService sysDepartService = (ISysDepartService) SystemConfig.getBean("sysDepartServiceImpl");
        ...
        if (StringUtils.isEmpty(parentId)) {
            ...
            if (departList == null || departList.size() == 0) {
                strArray[0] = CodeGenerateUtils.getNextYouBianCode(null);
                strArray[1] = "1";
                return strArray;
            } else {
                ...
                newOrgCode = CodeGenerateUtils.getNextYouBianCode(oldOrgCode);
            }
            ...
            if (parentList == null || parentList.size() == 0) {
                // 直接生成当前的部门编码并返回
                newOrgCode = CodeGenerateUtils.getSubYouBianCode(parentCode, null);
            } else { //处理有同级部门的情况
                // 获取同级部门的编码,利用工具类
                String subCode = parentList.get(0).getOrgCode();
                // 返回生成的当前部门编码
                newOrgCode = CodeGenerateUtils.getSubYouBianCode(parentCode, subCode);
            }
        }
        // 返回最终封装了部门编码和部门类型的数组
