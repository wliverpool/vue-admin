...
import wfm.example.back.mapper.SysAnnouncementMapper;
...
import wfm.example.back.service.ISysAnnouncementService;
...
public class SysAnnouncementServiceImpl extends ServiceImpl<SysAnnouncementMapper, SysAnnouncement> implements ISysAnnouncementService {
    ...
    public boolean updateAnnouncement(SysAnnouncement sysAnnouncement) {
        ...
        String userId = sysAnnouncement.getUserIds();
        if(ObjectConvertUtils.isNotEmpty(userId)&&sysAnnouncement.getMsgType().equals(CommonConstant.MSG_TYPE_UESR)) {
            // 2.补充新的通知用户数据
            String[] userIds = userId.substring(0, (userId.length()-1)).split(",");
            String anntId = sysAnnouncement.getId();
            Date refDate = new Date();
