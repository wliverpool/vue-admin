...
public class JwQyMenuUtils {
    ...
    public static int createMenu(Menu menu, String accessToken, String agentid) {
        log.info("[CREATEMENU]", "createMenu param:accessToken:{},agentid:{},menu:{}", new Object[]{accessToken,agentid,menu});
        int result = 0;
        // 拼装创建菜单的url
        String url = menu_create_url.replace("ACCESS_TOKEN", accessToken).replace("AGENTID", agentid);
        // 将菜单对象转换成json字符串
        String jsonMenu = JSONObject.toJSONString(menu);
        log.info("[CREATEMENU]", "createMenu param:jsonMenu:{}", new Object[]{jsonMenu});
        // 调用接口创建菜单
        JSONObject jsonObject = HttpUtils.sendPost(url, jsonMenu);
        log.info("[CREATEMENU]", "createMenu response:{}", new Object[]{jsonObject.toJSONString()});
        if (null != jsonObject) {
    ...
    public static int deleteMenu(String accessToken,String agentid) {
        log.info("[DELETEMENU]", "deleteMenu param:accessToken:{},agentid:{}", new Object[]{accessToken,agentid});
        int result = 0;
        // 拼装删除菜单的url
        ...
        log.info("[DELETEMENU]", "deleteMenu response:{}", new Object[]{jsonObject.toJSONString()});
        if (null != jsonObject) {
            int errcode = jsonObject.getIntValue("errcode");
            result = errcode;
        }
    ...
    public static JSONObject getMenu(String accessToken,String agentid) {
        log.info("[GETMENU]", "getMenu param:accessToken:{},agentid:{}", new Object[]{accessToken,agentid});
        // 拼装获取菜单列表的url
        String url = menu_get_url.replace("ACCESS_TOKEN", accessToken).replace("AGENTID", agentid);
        // 调用接口获取菜单列表
        JSONObject jsonObject = HttpUtils.sendGet(url);
        log.info("[GETMENU]", "getMenu response:{}", new Object[]{jsonObject.toJSONString()});
        return jsonObject;
    }
    ...
    public static void main(String[] args){
        try {
            ...
            //创建菜单
            JwQyMenuUtils.createMenu(menu, accessToken.getAccesstoken(), JwParames.agentid);
