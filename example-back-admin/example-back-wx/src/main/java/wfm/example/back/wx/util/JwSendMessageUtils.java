...
import wfm.example.back.wx.model.user.Group;
import wfm.example.back.wx.model.wxmedia.WxArticlesRequest;
import wfm.example.back.wx.model.wxsendmsg.*;
...
@Slf4j
public class JwSendMessageUtils {
    ...
    public static String uploadArticleImgs(String content,String accesstoken,String baseImageUrl,String domain){
        try {
            String[] urls = ReadImgUtils.getImgs(content);
            for(String imgurl:urls){
                ...
                String newUrl = JwSendMessageUtils.uploadImg(accesstoken, tempimgurl);
        ...
        if (accesstoken != null) {
            ...
            try {
                ...
                if (result.getInt("errcode") != 0) {
                    log.error("多媒体消息预览失败！errcode=" + result.getString("errcode") + ",errmsg = " + result.getString("errmsg"));
