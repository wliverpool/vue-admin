...
import wfm.example.back.wx.model.ai.Voice;
...
public class JwAiUtils {
        ...
        if(!file.exists()||!file.isFile()){
            ...
            try {
                ...
                try {
                    ...
                    log.info("--提交语音接口返回："+buffer.toString());
                } catch (Exception e) {
    ...
    private static String getVoiceResult(String accessToken,String voice_id,String lang){
        ...
        log.info("--获取语音识别结果请求参数："+requestUrl);
        JSONObject result =httpRequest(requestUrl, "POST",null);
        if(result!=null){
            log.info("--获取语音识别结果---"+result);
        }else{
            log.info("--获取语音识别结果为空---");
        }
