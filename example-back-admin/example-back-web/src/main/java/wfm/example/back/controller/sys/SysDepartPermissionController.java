...
import wfm.example.back.sys.service.ISysDepartRolePermissionService;
...
public class SysDepartPermissionController {
    ...
    public Result<Map<String,Object>> queryTreeListForDeptRole(@RequestParam(name="departId",required=true) String departId,HttpServletRequest request) {
        ...
        try {
            ...
            getTreeVoList(treeList, list, null);
    ...
    private void getTreeVoList(List<TreeVo> treeList, List<SysPermission> metaList, TreeVo temp) {
        for (SysPermission permission : metaList) {
            String tempPid = permission.getParentId();
            TreeVo tree = new TreeVo(permission.getId(), tempPid, permission.getName(),permission.getRuleFlag(), permission.isLeaf());
            if(temp==null && ObjectConvertUtils.isEmpty(tempPid)) {
                treeList.add(tree);
                if(!tree.getIsLeaf()) {
                    getTreeVoList(treeList, metaList, tree);
                }
