...
public class SysDepartController {
    ...
    public Result<List<SysDepartTreeDTO>> queryMyDeptTreeList(@AuthenticationPrincipal UserDetails userDetails) {
        ...
        try {
            if(ObjectConvertUtils.isNotEmpty(user.getUserIdentity()) && user.getUserIdentity().equals( CommonConstant.USER_IDENTITY_2 )){
                List<SysDepartTreeDTO> list = sysDepartService.queryMyDeptTreeList(user.getDepartIds());
    ...
    public Result<List<SysDepartTreeDTO>> queryTreeList() {
        Result<List<SysDepartTreeDTO>> result = new Result<>();
        try {
            // 从内存中读取
//			List<SysDepartTreeDTO> list =FindsDepartsChildrenUtil.getSysDepartTreeList();
//			if (CollectionUtils.isEmpty(list)) {
//				list = sysDepartService.queryTreeList();
//			}
            List<SysDepartTreeDTO> list = sysDepartService.queryTreeList();
            result.setResult(list);
            result.setSuccess(true);
        } catch (Exception e) {
            log.error(e.getMessage(),e);
    ...
    public Result<SysDepart> add(@RequestBody SysDepart sysDepart, HttpServletRequest request) {
        ...
        try {
            ...
            // FindsDepartsChildrenUtil.clearDepartIdDTO();
            result.success("添加成功！");
        } catch (Exception e) {
            log.error(e.getMessage(),e);
    ...
    public Result<List<DepartIdDTO>> queryIdTree() {
//		Result<List<DepartIdDTO>> result = new Result<List<DepartIdDTO>>();
//		List<DepartIdDTO> idList;
//		try {
//			idList = FindsDepartsChildrenUtil.wrapDepartIdDTO();
...
//		}
        ...
        try {
            List<DepartIdDTO> list = sysDepartService.queryDepartIdTreeList();
            result.setResult(list);
            result.setSuccess(true);
        } catch (Exception e) {
            log.error(e.getMessage(),e);
    ...
    public Result<List<SysDepartTreeDTO>> searchBy(@RequestParam(name = "keyWord", required = true) String keyWord, @RequestParam(name = "myDeptSearch", required = false) String myDeptSearch,@AuthenticationPrincipal UserDetails userDetails) {
        Result<List<SysDepartTreeDTO>> result = new Result<List<SysDepartTreeDTO>>();
        ...
        List<SysDepartTreeDTO> treeList = this.sysDepartService.searchBy(keyWord,myDeptSearch,departIds);
    ...
    public Result<Map<String,Object>> queryTreeByKeyWord(@RequestParam(name = "keyWord", required = false) String keyWord) {
        ...
        try {
            ...
            List<SysDepartTreeDTO> list = sysDepartService.queryTreeByKeyWord(keyWord);
            //根据keyWord获取用户信息
