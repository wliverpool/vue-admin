...
import wfm.example.back.sys.service.ISysPermissionDataRuleService;
import wfm.example.back.sys.service.ISysPermissionService;
import wfm.example.back.sys.service.ISysRolePermissionService;
import wfm.example.back.util.PermissionDataUtil;
import wfm.example.back.util.SysPermissionUtil;
import wfm.example.back.vo.SysPermissionVo;
...
public class SysPermissionController {
    ...
    public Result<List<SysPermissionVo>> list() {
        ...
        try {
            ...
            List<SysPermissionVo> treeList = new ArrayList<>();
    ...
    public Result<List<SysPermissionVo>> getSystemMenuList() {
        long start = System.currentTimeMillis();
        Result<List<SysPermissionVo>> result = new Result<>();
        try {
            LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
            ...
            query.orderByAsc(SysPermission::getSortNo);
            List<SysPermission> list = sysPermissionService.list(query);
            List<SysPermissionVo> sysPermissionTreeList = new ArrayList<SysPermissionVo>();
            for(SysPermission sysPermission : list){
                SysPermissionVo sysPermissionTree = new SysPermissionVo(sysPermission);
    ...
    public Result<List<SysPermissionVo>> getSystemSubmenu(@RequestParam("parentId") String parentId){
        Result<List<SysPermissionVo>> result = new Result<>();
        try{
            LambdaQueryWrapper<SysPermission> query = new LambdaQueryWrapper<SysPermission>();
    ...
    public Result getSystemSubmenuBatch(@RequestParam("parentIds") String parentIds) {
        try {
            ...
            query.orderByAsc(SysPermission::getSortNo);
            List<SysPermission> list = sysPermissionService.list(query);
            Map<String, List<SysPermissionVo>> listMap = new HashMap<>();
            for (SysPermission item : list) {
                String pid = item.getParentId();
                if (parentIdList.contains(pid)) {
                    List<SysPermissionVo> mapList = listMap.get(pid);
                    if (mapList == null) {
                        mapList = new ArrayList<>();
                    }
                    mapList.add(new SysPermissionVo(item));
    ...
    public Result<?> getUserPermissionByToken(@AuthenticationPrincipal UserDetails user) {
        Result<JSONObject> result = new Result<JSONObject>();
        try {
            log.info(" ------ 获取用户:{},拥有的访问菜单 ------ ", user.getUsername());
    ...
    public Result<SysPermission> deleteBatch(@RequestParam(name = "ids", required = true) String ids) {
        ...
        try {
            ...
            for (String id : arr) {
                if (ObjectConvertUtils.isNotEmpty(id)) {
                    sysPermissionService.deletePermission(id);
                }
            }
            result.success("删除成功!");
    ...
    public Result<Map<String, Object>> queryTreeList() {
        ...
        try {
            ...
            getTreeDTOList(treeList, list, null);
    ...
    public Result<List<TreeVo>> queryAsync(@RequestParam(name = "pid", required = false) String parentId) {
        Result<List<TreeVo>> result = new Result<>();
        try {
            List<TreeVo> list = sysPermissionService.queryListByParentId(parentId);
    ...
    private void getPermissionJsonArray(JSONArray jsonArray, List<SysPermission> metaList, JSONObject parentJson) {
        for (SysPermission permission : metaList) {
            ...
            if (parentJson == null && ObjectConvertUtils.isEmpty(tempPid)) {
                jsonArray.add(json);
                if (!permission.isLeaf()) {
                    getPermissionJsonArray(jsonArray, metaList, json);
                }
            } else if (parentJson != null && ObjectConvertUtils.isNotEmpty(tempPid) && tempPid.equals(parentJson.getString("id"))) {
    ...
    private JSONObject getPermissionJsonObject(SysPermission permission) {
        ...
        if (permission.getMenuType().equals(CommonConstant.MENU_TYPE_2)) {
            ...
            if (ObjectConvertUtils.isNotEmpty(permission.getComponentName())) {
            ...
            if (ObjectConvertUtils.isEmpty(permission.getParentId())) {
                // 一级菜单跳转地址
                json.put("redirect", permission.getRedirect());
                if (ObjectConvertUtils.isNotEmpty(permission.getIcon())) {
    ...
    private String urlToRouteName(String url) {
        if (ObjectConvertUtils.isNotEmpty(url)) {
            if (url.startsWith("/")) {
                url = url.substring(1);
            }
            url = url.replace("/", "-");
    ...
    private void getTreeDTOList(List<TreeVo> treeList, List<SysPermission> metaList, TreeVo temp) {
        for (SysPermission permission : metaList) {
            String tempPid = permission.getParentId();
            TreeVo tree = SysPermissionUtil.convertTreeVo(permission);
            if (temp == null && ObjectConvertUtils.isEmpty(tempPid)) {
                treeList.add(tree);
                if (!tree.getIsLeaf()) {
                    getTreeDTOList(treeList, metaList, tree);
                }
    ...
    private void getTreeList(List<SysPermissionVo> treeList, List<SysPermission> metaList, SysPermissionVo temp) {
        for (SysPermission permission : metaList) {
            String tempPid = permission.getParentId();
            SysPermissionVo tree = new SysPermissionVo(permission);
            if (temp == null && ObjectConvertUtils.isEmpty(tempPid)) {
                treeList.add(tree);
                if (!tree.isLeaf()) {
