...
public class SysUserController {
    ...
    public Result<SysUser> add(@RequestBody JSONObject jsonObject) {
        ...
        try {
            ...
            // PasswordUtils.encrypt(user.getUsername(), user.getPassword(), salt);
    ...
    public Result<List<DepartIdDTO>> getUserDepartsList(@RequestParam(name = "userId", required = true) String userId) {
        Result<List<DepartIdDTO>> result = new Result<>();
        try {
            List<DepartIdDTO> depIdModelList = this.sysUserDepartService.queryDepartIdsOfUser(userId);
    ...
    public Result<IPage<SysUser>> departUserList(@RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
                                                 @RequestParam(name="pageSize", defaultValue="10") Integer pageSize, HttpServletRequest req, @AuthenticationPrincipal UserDetails userDetails) {
        Result<IPage<SysUser>> result = new Result<IPage<SysUser>>();
        Page<SysUser> page = new Page<SysUser>(pageNo, pageSize);
        ...
        //部门id为空时，查询我的部门下所有用户
        if(ObjectConvertUtils.isEmpty(depId)){
            JwtUser user = (JwtUser) userDetails;
            int userIdentity = user.getUserIdentity() != null?user.getUserIdentity():CommonConstant.USER_IDENTITY_1;
            if(ObjectConvertUtils.isNotEmpty(userIdentity) && userIdentity == CommonConstant.USER_IDENTITY_2 ){
    ...
    public Result<?> queryByDepartId(
        ...
        IPage<SysUserSysDepartDTO> pageList = sysUserService.queryUserByOrgCode(orgCode, userParams, new Page(pageNo, pageSize));
    ...
    public Result<?> queryByOrgCodeForAddressList(
        ...
        IPage<SysUserSysDepartDTO> pageList = sysUserService.queryUserByOrgCode(orgCode, userParams, page);
        List<SysUserSysDepartDTO> list = pageList.getRecords();
        ...
        for (SysUserSysDepartDTO item : list) {
...
//                json.put("avatar", item.getSysUser().getAvatar());
    ...
    public Result<Map<String,Object>> getCurrentUserDeparts(@AuthenticationPrincipal UserDetails user) {
    ...
    public Result<JSONObject> userRegister(@RequestBody JSONObject jsonObject, SysUser user) {
        ...
        Object code = redisDao.get(phone);
        ...
        if(ObjectConvertUtils.isNotEmpty(email)){
        ...
        try {
            ...
            // PasswordUtils.encrypt(username, password, salt);
            user.setSalt(salt);
    ...
    public Result<Map<String,String>> phoneVerification(@RequestBody JSONObject jsonObject) {
        ...
        //设置有效时间
        redisDao.set(phone, smscode,600);
        //新增查询用户名
    ...
    public Result<SysUser> passwordChange(@RequestParam(name="username")String username,
        ...
        Result<SysUser> result = new Result<SysUser>();
        if(ObjectConvertUtils.isEmpty(username) || ObjectConvertUtils.isEmpty(password) || ObjectConvertUtils.isEmpty(smscode)  || ObjectConvertUtils.isEmpty(phone) ) {
        ...
        Object object= redisDao.get(phone);
        if(null==object) {
        ...
        if (sysUser == null) {
            ...
            String passwordEncode = passwordEncoder.encode(password);
            // PasswordUtils.encrypt(sysUser.getUsername(), password, salt);
    ...
    public Result<?> appUserList(@RequestParam(name = "keyword", required = false) String keyword,
        ...
        try {
            ...
            if(ObjectConvertUtils.isNotEmpty(syncFlow)){
    ...
    public Result<SysUser> appEdit(HttpServletRequest request,@RequestBody JSONObject jsonObject) {
        ...
        try {
            ...
            SysUser sysUser = sysUserService.getUserByName(username);
            log.info("移动端编辑用户，id： " +jsonObject.getString("id") ,CommonConstant.LOG_TYPE_2, 2);
            if(sysUser==null) {
