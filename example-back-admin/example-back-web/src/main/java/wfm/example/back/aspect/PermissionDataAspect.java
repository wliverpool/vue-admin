package wfm.example.back.aspect;
...
public class PermissionDataAspect {
    ...
    @Pointcut("@annotation(wfm.example.back.common.aspect.PermissionData)")
    ...
    public Object arround(ProceedingJoinPoint point) throws  Throwable{
        ...
        if(ObjectConvertUtils.isNotEmpty(component)) {
            ...
            /*if(currentSyspermission==null || currentSyspermission.size()==0) {
                ...
                if(ObjectConvertUtils.isNotEmpty(userMatchUrl)){
        ...
        if(currentSyspermission!=null && currentSyspermission.size()>0) {
            ...
            List<SysPermissionDataRuleDTO> dataRules = new ArrayList<SysPermissionDataRuleDTO>();
            for (SysPermission sysPermission : currentSyspermission) {
                ...
                if(temp!=null && temp.size()>0) {
                    ...
                    dataRules = ObjectConvertUtils.entityListToModelList(temp,SysPermissionDataRuleDTO.class);
                }
                // update-end--Author:scott Date:20191119 for：数据权限规则编码不规范，项目存在相同包名和类名 #722
            }
            if(dataRules!=null && dataRules.size()>0) {
                DataAuthorUtils.installDataSearchConditon(request, dataRules);
                SysUserCacheVo userinfo = sysUserService.getCacheUser(username);
                DataAuthorUtils.installUserInfo(request, userinfo);
            }
        }
    ...
    private String filterUrl(String requestPath){
        String url = "";
        if(ObjectConvertUtils.isNotEmpty(requestPath)){
    ...
    private String getJgAuthRequsetPath(HttpServletRequest request) {
        ...
        if(ObjectConvertUtils.isNotEmpty(queryString)){
